...
-- Add required flag to quiz_questions table if it doesn't exist
ALTER TABLE public.quiz_questions 
ADD COLUMN IF NOT EXISTS is_required BOOLEAN DEFAULT true;

-- Update the process_quiz_tags function to handle range values and required validation
CREATE OR REPLACE FUNCTION public.process_quiz_tags(
  p_user_id UUID,
  p_responses JSONB
)
RETURNS VOID AS $$
...
-- Create function to validate quiz responses against required questions
CREATE OR REPLACE FUNCTION public.validate_quiz_responses(
  p_responses JSONB
)
RETURNS TABLE(is_valid BOOLEAN, missing_questions TEXT[]) AS $$
DECLARE
