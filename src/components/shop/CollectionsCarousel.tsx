...
const CollectionsCarousel = ({
  ...
  const getCollectionProductCount = (collectionId: string) => {
    return products.filter(product => product.product_collections?.some((pc: any) => pc.collection_id === collectionId)).length;
  };
  const getRecommendedProductCount = (collectionId: string) => {
    ...
    return products.filter(product => product.product_collections?.some((pc: any) => pc.collection_id === collectionId) && personalizedProductIds.has(product.id)).length;
  };
  if (loading) {
    return <div className="mb-12">
        <h2 className="text-2xl font-bold text-forest mb-6">{title || 'Shop by Collection'}</h2>
  ...
  return <div className="mb-12">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-neutral-950">{title || 'Shop by Collection'}</h2>
        {displayCollections.length > 4 && <div className="hidden md:flex space-x-2">
      ...
      <div ref={scrollRef} className="flex gap-4 overflow-x-auto scrollbar-hide pb-2 snap-x snap-mandatory md:overflow-hidden" style={{
      scrollbarWidth: 'none',
      msOverflowStyle: 'none'
        ...
        return <div key={collection.id} className="flex-none w-64 snap-start">
              <Card className="group cursor-pointer hover:shadow-lg transition-all duration-300 h-full relative" onClick={() => handleCollectionClick(collection.id)}>
                <CardContent className="p-6 text-center h-full flex flex-col justify-center">
                  ...
                  {collection.description && <p className="text-sm text-muted-foreground line-clamp-2 mb-4">
                  ...
                  <div className="flex items-center justify-center gap-4 mt-auto">
                    ...
                    {user && !personalizedLoading && recommendedProducts > 0 && <div className="flex items-center gap-1 bg-emerald-50 rounded-full px-[8px]">
